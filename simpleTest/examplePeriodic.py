...
def TestFunction(x):
    return np.sin(4*np.pi*x[0]*x[4]) + 2 - np.exp(np.sin(2*np.pi*x[3])) + np.sin(4*np.pi*x[5]*x[1])**2
...
b   =  M / (math.log10(M) * num)  # number for the number of frequencies if we use logarithmic oversampling and distribute it evenly to all subsets
bw  = np.array([math.floor(b/2)*2, math.floor(math.sqrt(b)/2)*2]) # bandwidths (use even numbers)
...
y      = np.array([TestFunction(X[i, :].T) for i in range(M)], dtype=complex)           # evaluate the function at these points
X_test = rng.random((M_test, d)) - 0.5                                                  #
y_test = np.array([TestFunction(X_test[i, :].T) for i in range(M_test)], dtype=complex) # the same for the test points
...
λ_min = min(mse, key=mse.get)  # get the regularisation parameter which leads to the minimal error
mse_min = mse[λ_min]
...
plt.show()
print("important dimensional interactions: : "+str([label[i] for i in np.arange(0,l)[gsis>1e-2]]))
...
N     = np.array([math.floor(bws**(1/max(1,len(u)))/2)*2 for u in U]) # distribute the frequencies evenly and make them even
N[0]  = 0

a = ANOVAapprox.approx(X, y, U, N, "per") # generate the data structure for the approximation
a.approximate(lam=lambdas, solver="lsqr") # do the approximation for all specified regularisation parameters
