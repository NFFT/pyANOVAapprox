...
types = {
        "per": complex, "cos": float, "cheb": float, "std": float, "chui1": float,
        "chui2": float, "chui3": float, "chui4": float, "mixed": complex,
        }
gt_systems = {
                "per": "exp", "cos": "cos", "cheb": "cos", "std": "cos", "chui1": "chui1",
                "chui2": "chui2", "chui3": "chui3", "chui4": "chui4", "mixed": "mixed",
                } 
...
class approx:
    def __init__(self, X, y, U = None, N = np.array([]), basis="cos", classification=False, basis_vect=[], fastmult=None, ds = None):


        if len(N) == 0:
            ValueError("please define N")

        if U == None or len(U) == 0:   #setting U   #approx(X::Matrix{Float64}, y::Union{Vector{ComplexF64},Vector{Float64}}, ds::Int, N::Vector{Int}, basis::String = "cos"; classification::Bool = false, basis_vect::Vector{String} = Vector{String}([]), fastmult::Bool = classification ? true : false,)
            U = get_superposition_set(X.shape[1], ds)
            
        if N.ndim == 1:    #setting N    #approx( X::Matrix{Float64}, y::Union{Vector{ComplexF64},Vector{Float64}}, U::Vector{Vector{Int}}, N::Vector{Int}, basis::String = "cos"; classification::Bool = false, basis_vect::Vector{String} = Vector{String}([]), fastmult::Bool = classification ? true : false,)
        ...
        if y[0].dtype != vtypes[basis]:
            raise TypeError("Periodic functions require complex vectors, nonperiodic functions real vectors.")
        ...
        for i in range(len(U)):
            ...
            if len(u) > 0:
                if len(N[i]) != len(u):
                    raise ValueError(f"Vector N has for the set {u} not the right length")
    ...
    def Approximate(self, lam, max_iter=50, weights=None, verbose=False, solver=None, tol=1e-8):   #helpfuntion for approximate
        M = self.X.shape[0]
        ...
        if self.classification and solver != "fista":
            raise ValueError("Classification is only implemented with the fista solver.")

        if solver == "lsqr":    
            diag_w_sqrt = np.sqrt(lam) * np.sqrt(w)

            def matv(fhat):
                return np.concatenate((self.trafo @ GroupedCoefficients(self.trafo.settings, fhat), diag_w_sqrt * fhat))

            def rmatv(f):
                return (self.trafo.H @ f[:M]).vec() + (diag_w_sqrt * f[M:])

            F_vec = DeferredLinearOperator(mfunc = matv, rmfunc = rmatv, shape = (M+nf, nf), dtype = types[self.basis])

            tmp = lsqr(F_vec, np.concatenate((self.y, np.zeros(nf, dtype=types[self.basis]))), atol=tol, btol=tol, iter_lim=max_iter, show=verbose)     
            self.fc[lam] = GroupedCoefficients(self.trafo.settings, tmp[0])

        elif solver == "fista":
            ghat = GroupedCoefficients(self.trafo.settings, tmp)
            fista(ghat, self.trafo, self.y, lam, what, max_iter = max_iter, classification = self.classification)
            self.fc[lam] = ghat
        else:
            raise ValueError("Solver not found.")

    def approximate(self, lam, max_iter=50, weights=None, verbose=False, solver=None, tol=1e-8):
        """
        ...
        """
        if isinstance(lam, np.ndarray) and lam.dtype == float:   #approximate( a::approx; lambda::Vector{Float64} = exp.(range(0, 5, length = 5)), max_iter::Int = 50, weights::Union{Vector{Float64},Nothing} = nothing, verbose::Bool = false, solver::String = "lsqr" )::Nothing
            lam = np.sort(lam)[::-1]
            for l in lam:
                self.Approximate(l, max_iter=max_iter, weights=weights, verbose=verbose, solver=solver, tol=tol)

        elif isinstance(lam, float): # approximate( a::approx, λ::Float64; max_iter::Int = 50, weights::Union{Vector{Float64},Nothing} = nothing, verbose::Bool = false, solver::String = "lsqr", tol:.Float64b= 1e-8 )::Nothing
            self.Approximate(lam, max_iter=max_iter, weights=weights, verbose=verbose, solver=solver, tol=tol)

        else:
            raise ValueError(lam, "has to be a float or a numpy array with float as dtype") 
    ...
    def evaluate(self, lam = None, X = None):
        ...
        if X is not None:
            ...
            trafo = GroupedTransform(system = gt_systems[self.basis], U = self.U, N = self.N, X = Xt, basis_vect=self.basis_vect)

            if lam is not None: #evaluate( a::approx; X::Matrix{Float64}, λ::Float64 )::Union{Vector{ComplexF64},Vector{Float64}}
            ...
            if lam is not None: #evaluate( a::approx; λ::Float64 )::Union{Vector{ComplexF64},Vector{Float64}}
    ...
    def evaluateANOVAterms(self, X, lam = None):    
        ...
        if lam is not None:   #evaluateANOVAterms( a::approx; X::Matrix{Float64}, λ::Float64 )::Union{Matrix{ComplexF64},Matrix{Float64}}
    ...
    def evaluateSHAPterms(self, X, lam = None):
        ...
        if lam is not None:  #evaluateSHAPterms( a::approx; X::Matrix{Float64}, λ::Float64 )::Union{Matrix{ComplexF64},Matrix{Float64}}
